...
type DropdownImage = { title: string; img: string; link?: string };

const SezaneNavbar = () => {
  const [activeDropdown, setActiveDropdown] = useState<string | null>(null);
  const [shopImages, setShopImages] = useState<DropdownImage[]>([]);
  const [newInImages, setNewInImages] = useState<DropdownImage[]>([]);
          ...
          <div className="flex items-center justify-center gap-8 h-16">
            {navLinks.map((item) => (
              <div
                key={item.label}
                className="relative"
                onMouseEnter={() => dropdownShouldOpen(item.label) && handleMouseEnter(item.label)}
                onMouseLeave={handleMouseLeave}
              >
                <div className="group relative cursor-pointer py-2">
                  {item.href ? (
                    <Link href={item.href}>
                      <span className="font-['Oswald'] text-white hover:text-black font-semibold tracking-wider transition-colors text-sm uppercase">
                        {item.label}
                      </span>
                    </Link>
                  ) : (
          ...
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex gap-12">
              ...
              <div className="flex-[2] flex gap-4">
                {(activeDropdown === 'SHOP' ? shopImages : activeDropdown === 'NEW IN' ? newInImages : []).map(({ title, img, link }, idx) => (
                  <div
                    key={idx}
